...
// GETCURRENTEVENTS GOALS:
// call the mapCurrentEvents function and send the result to the front-end

function getCurrentEvents(request, response){
  response.status(200).send(mapCurrentEvents());
}

//-- MAPCURRENTEVENTS GOALS: 
// maps over the currentEvents Object, runs it through a constructor, and returns the resulting Array.
...
// split the string in order to create an array
// now we can run .reduce on the new array
// string is the accumulator 
// element is the current value
// beginning at '' because if you start with a string you will end with a string
// 
/* ------------------------------------------------------------------------------------------------
